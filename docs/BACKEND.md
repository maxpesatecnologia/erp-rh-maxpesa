# Documentação técnica — backend, Supabase e integrações

Este documento reúne o detalhamento técnico módulo a módulo: schema SQL, políticas de RLS e as decisões de
implementação que não são óbvias só lendo o código. Para a visão geral do projeto e como rodá-lo, veja o
[README.md](../README.md). Para o guia de uso das telas, veja o [Manual do Usuário](../MANUAL_DO_USUARIO.md).

## Índice

- [Ligando o Supabase (passo a passo completo)](#ligando-o-supabase-passo-a-passo-completo)
- [Admissão Digital é alimentada pelo Recrutamento](#admissão-digital-é-alimentada-pelo-recrutamento)
- [Aba Documentos: repositório de documentos por colaborador](#aba-documentos-repositório-de-documentos-por-colaborador)
- [Dependentes nomeados no Cadastro de Colaboradores](#dependentes-nomeados-no-cadastro-de-colaboradores)
- [Desligamento Digital é a lógica inversa da Admissão](#desligamento-digital-é-a-lógica-inversa-da-admissão)
- [Gestão de Férias](#gestão-de-férias)
- [Comunicação Interna: mural, aniversariantes e férias da equipe](#comunicação-interna-mural-aniversariantes-e-férias-da-equipe)
- [Avaliação de Desempenho (proposta em validação com a gestão)](#avaliação-de-desempenho-proposta-em-validação-com-a-gestão)
- [Avaliação das Equipes](#avaliação-das-equipes-status-do-time-reportado-pelo-gestor-ao-rh)
- [Auditoria: quem editou por último e logs completos para o admin](#auditoria-quem-editou-por-último-e-logs-completos-para-o-admin)
- [Convenção de prefixo `rh_`](#convenção-de-prefixo-rh-banco-compartilhado-entre-sistemas)
- [Login restrito a e-mails pré-liberados](#login-restrito-a-e-mails-pré-liberados)
- [IA Corporativa: assistente de RH/DP com a API do Google Gemini](#ia-corporativa-assistente-de-rhdp-com-a-api-do-google-gemini)
- [Sobre os dados das telas (hoje mockados, no formato do SharePoint)](#sobre-os-dados-das-telas-hoje-mockados-no-formato-do-sharepoint)
- [Estrutura do projeto](#estrutura-do-projeto)
- [Módulos existentes no código mas ainda não conectados](#módulos-existentes-no-código-mas-ainda-não-conectados)
- [Próximos passos sugeridos](#próximos-passos-sugeridos)

## Ligando o Supabase (passo a passo completo)
...
5. Crie os usuários em **Authentication → Users** (o app não tem tela de cadastro público) e depois insira a
   ...
   acesso: sem ela, o login é recusado mesmo com e-mail/senha corretos (ver seção "Login restrito" abaixo).
...
Desde a funcionalidade de **editar a data de demissão pelo próprio Cadastro de Colaboradores** (colaborador já
desligado), existe também `sincronizarDataDesligamento` em `src/lib/colaboradoresApi.js`: ao salvar um
colaborador com status "Desligado" e uma `data_demissao` nova/alterada, essa função cria (se não existir) ou
atualiza a `data_desligamento` da linha correspondente em `rh_desligamentos`, mantendo os dois módulos
sincronizados sem exigir uma segunda edição manual.
...
Os painéis de autoatendimento do colaborador no Portal (`src/pages/portal/panels/SolicitarFeriasPanel.jsx` /
`SaldoFeriasPanel.jsx`) existem no código mas **não estão roteados/conectados** hoje (ver
[Módulos existentes no código mas ainda não conectados](#módulos-existentes-no-código-mas-ainda-não-conectados)):
eles dependem de existir um vínculo entre o usuário autenticado (`rh_profiles`) e sua matrícula em
`rh_colaboradores`, que hoje não existe em nenhum módulo do Portal — é um pré-requisito maior, compartilhado com
os outros painéis de autoatendimento (dados cadastrais, banco de horas etc.), não só o de férias.
...
Não existe hoje um vínculo de "gestor responsável por este time" no cadastro — qualquer `admin`/`rh`/`gestor` pode
registrar avaliação para qualquer time; o nome de quem registrou fica salvo em cada avaliação
(`gestor_responsavel`, preenchido com o nome do usuário logado) e a leitura é livre para todo mundo que acessa
`/equipes`.
...
Duas integrações **read-only** já existem no código de verdade (não são mock) para o sistema externo "EPI
Controle": **Gestão de ASOs** (`/medicina`) e **Gestão de EPIs** (`/epis`) chamam essa API e caem em dados de
exemplo com aviso na tela apenas se a integração não estiver configurada.
...
src/
  ...
  lib/             supabaseClient, *Api.js (uma camada de dados por módulo), motor de respostas da IA Corporativa
...
## Módulos existentes no código mas ainda não conectados

Alguns arquivos já existem no repositório mas **não têm rota nem entrada de menu** — não aparecem no app hoje.
Ao trabalhar neles, primeiro cheque `src/App.jsx` e `src/config/modules.js` para confirmar o estado atual antes
de assumir que estão em produção:

- `src/pages/seguranca/SegurancaTrabalho.jsx` — protótipo visual (dados mock, botões "Editar" desabilitados de
  propósito) de um futuro módulo de Segurança do Trabalho (PGR/PCMSO, APRs, DDS, inspeções, não conformidades,
  CATs).
- `src/pages/treinamentos/` — página standalone de Treinamentos (cadastro por nome livre, não vinculado ao
  Cadastro de Colaboradores).
- `src/pages/portal/panels/*` (DadosCadastrais, EnviarDocumentos, AssinarDocumentos, BancoHoras, SaldoFerias,
  SolicitarFerias, Comunicados, Treinamentos, SolicitacaoRH) — protótipo de um portal de autoatendimento mais
  completo para o colaborador. Hoje `/portal` renderiza só `PortalColaborador.jsx` (um grid de atalhos para os
  módulos que o perfil logado pode acessar). Ligar esses painéis de verdade depende de existir um vínculo entre
  o usuário autenticado (`rh_profiles`) e sua matrícula em `rh_colaboradores` — pré-requisito ainda não
  implementado em nenhum lugar do sistema.
...
- [ ] Definir se/quando o portal de autoatendimento do colaborador (painéis em
      `src/pages/portal/panels/`) e o módulo de Segurança do Trabalho serão conectados de verdade.
